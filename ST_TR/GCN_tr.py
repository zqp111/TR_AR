...
class unit_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, groups, coff_embedding=4, num_subset=3):
        ...
        self.DecoupleA = nn.Parameter(torch.tensor(
            np.reshape(A.astype(np.float32),[3,1,25,25]), 
            dtype=torch.float32, requires_grad=True).repeat(1,groups,1,1), 
            requires_grad=True)
...
class MuiltKernelGTCN_(nn.Module):
    def __init__(self, in_channels,
        ...
        self.tcn0 = nn.Sequential(
            ...
            #           stride=(stride, 1),
            #           padding=padding),
        self.tcn1 = TransformerEncoder(hidden_size=self.out_channels,
                                ff_size=self.out_channels*4, 
    ...
    def forward(self, x):
        ...
        x = self.tcn0(x)
        bs, F, T, N = x.shape
        x = x.permute(0, 3, 2, 1).contiguous().view(bs*N, T, F)
        x = self.tcn1(x, T, None)
        ...
        if self.stride > 1:
            ...
            x = x.permute(0, 2, 1)
            x = nn.functional.max_pool1d(x, self.stride)
            x = x.permute(0, 2, 1)
            _, T, _ = x.shape

        x = x.view(bs, N, T, F).permute(0, 3, 2, 1).contiguous()
        # x = self.tcn2(x)
        x = self.relu(x)
        return x