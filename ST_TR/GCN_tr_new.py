...
class MuiltKernelGTCN_tr(nn.Module):
    def __init__(self, in_channels,
        ...
        self.gcn = MultiHeadedGraphAttention(input_size=in_channels, num_heads=num_heads, size=out_channels, dropout=dropout)
    ...
    def forward(self, x):
        ...
        # print(x.shape)
        x = x.permute(0, 2, 3, 1).contiguous().view(bs*T, N, F)
        x = self.gcn(x, x, x)
        x = x.view(bs, T, N, -1).permute(0, 3, 1, 2).contiguous()
        x += residual
        # print(x.shape)
        x = self.tcn(x)
        x = self.relu(x)
